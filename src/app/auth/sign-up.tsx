...
export default function SignUpScreen() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (signUpError) {
        setError(signUpError.longMessage ?? signUpError.message ?? "Something went wrong.");
        return;
      }

      // verifications.sendEmailCode is the correct Core 3 API
  ...
  const handleVerify = async (code: string) => {
    ...
    // verifications.verifyEmailCode is the correct Core 3 API
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FFFFFF" }}>
      ...
      >
        ...
        >
          ...
          {/* Social Buttons */}
          <SocialButton icon={<GoogleIcon />} label="Continue with Google" onPress={() => handleSocialSignUp("oauth_google")} />
          <SocialButton icon={<FacebookIcon />} label="Continue with Facebook" onPress={() => handleSocialSignUp("oauth_facebook")} />
          <SocialButton icon={<AppleIcon />} label="Continue with Apple" onPress={() => handleSocialSignUp("oauth_apple")} />

          {/* Footer */}
          <View className="flex-row justify-center items-center mt-8 mb-4">
            <Text className="text-[15px] text-[#9090A8]">Already have an account? </Text>
            <TouchableOpacity onPress={() => router.push("/auth/sign-in")}>
              <Text className="text-[15px] text-[#6B4EFF] font-semibold">Log in</Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  inputContainer: {
    ...
    paddingTop: 12, paddingBottom: 12, borderWidth: 1, borderColor: "#EBEBF0",
    shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1,
  },
  inputLabel: { fontSize: 12, color: "#9090A8", fontWeight: "500", marginBottom: 4 },
  input: { fontSize: 16, color: "#1C1C1E", padding: 0, margin: 0 },
  passwordRow: { flexDirection: "row", alignItems: "center" },
  eyeButton: { paddingLeft: 8, paddingVertical: 2 },
  primaryButton: {
    backgroundColor: "#6B4EFF", borderRadius: 16, height: 56,
    alignItems: "center", justifyContent: "center", marginTop: 20,
    shadowColor: "#6B4EFF", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4,
  },
  ...
  socialButton: {
    ...
    borderRadius: 16, height: 56, paddingHorizontal: 20, marginBottom: 10,
